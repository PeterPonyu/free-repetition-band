...
def test_build_resolves_site_data_paths() -> None:
    ...
    assert (site / "data" / "figs" / "summaries" / "E1_scale_band.json").is_file()
    assert (site / "data" / "figs" / "summaries" / "E1_repeat.json").is_file()
    assert (site / "data" / "figs" / "summaries" / "E1_capxl.json").is_file()
